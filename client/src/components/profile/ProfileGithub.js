...
class ProfileGithub extends Component {
	constructor(props) {
		super(props);
		this.state = {
			clientId: 'd11141ae7497b7462a59',
			clientSecret: '3dd2535b1175c013eb63b2ad0a0ce06ce55a506a',
			repoCount: 5,
			sort: 'created: asc',
			repos: []
		}
	...
	componentDidMount() {
		...
		const {repoCount, sort, clientSecret, clientId} = this.state;

		fetch(`https://api.github.com/users/${username}/repos?per_page=${repoCount}&sort=${sort}&client_id=${clientId}&client_secret=${clientSecret}`)
			.then(res => res.json())
			.then(data => {
				if(this.refs.myRef) {
