import React, {Component} from 'react';
import Moment from 'react-moment';

// <Moment format="MM/DD/YYYY">{exp.from}</Moment>

class ProfileCreds extends Component {
	render() {
		const {experience, education} = this.props;
		const experienceMap = experience.map(exp => (
			<li className="list-group-item" key={exp._id}>
              <h4>{exp.company}</h4>
              <p><Moment format="MM/DD/YYYY">{exp.from}</Moment> - {exp.to === null ? ' Now' : <Moment format="MM/DD/YYYY">{exp.to}</Moment>}</p>
              <p><strong>Position:</strong> {exp.title}</p>
              <p>{exp.location === '' ? null : (<span><strong>Location:</strong> {exp.location}</span>)}</p>
              <p>{exp.description === '' ? null : (<span><strong>Description:</strong> {exp.description}</span>)}</p>
            </li>
		))

		const educationMap = education.map(edu => (
			<li className="list-group-item" key={edu._id}>
              <h4>{edu.school}</h4>
              <p><Moment format="MM/DD/YYYY">{edu.from}</Moment> - {edu.to === null ? ' Now' : <Moment format="MM/DD/YYYY">{edu.to}</Moment>}</p>
              <p><strong>Degree: </strong>{edu.degree}</p>
              <p><strong>Field Of Study: </strong>{edu.fieldofstudy}</p>
              <p>{edu.description === '' ? null : (<span><strong>Description:</strong> {edu.description}</span>)}</p>
            </li>
		))
		return (
			<div className="row">
	            <div className="col-md-6 my-3">
	              <h3 className="text-center text-info">Experience</h3>
	              {experienceMap.length > 0 ? (
	              	<ul className="list-group">{experienceMap}</ul>
	              ) : (
	              	<p className="text-center">No Experience Listed</p>
	              )}
	            </div>
	            <div className="col-md-6 my-3">
	              <h3 className="text-center text-info">Education</h3>
	              {educationMap.length > 0 ? (
	              	<ul className="list-group">{educationMap}</ul>
	              ) : (
	              	<p className="text-center">No Education Listed</p>
	              )}
	            </div>
	         </div>

		)
	}
}

export default ProfileCreds;